...
struct State {
    ...
    {   
        ...
        double background_scale_x = windowWidth / background.getSize().x; 
        double background_scale_y = windowHeight / background.getSize().y;
...
void updateGameOver(State& gs) {
    ...
    if(!gs.enemies.empty()) {
        for(auto& enemy : gs.enemies) {
            ...
            if(maxY > gs.windowHeight * 0.65) lost = true;
        }
    }
