...
void spawnBonusShip(State& gs) {
    ...
    if(!gs.existsBonusShip) {
        ...
        if(dirProb <= 50) {
            ...
            gs.bonusship.sprite.setPosition(sf::Vector2f(1280, distY));
        }
        ...
        if(!gs.bonusship.rightDirection && gs.bonusship.sprite.getPosition().x <= 0.0) { 
            gs.existsBonusShip = false;
        }
        else if(gs.bonusship.rightDirection && gs.bonusship.sprite.getPosition().x >= 1280.0) {
            gs.existsBonusShip = false;
        }
    ...
    if(gs.existsBonusShip) {
        ...
        for(auto& bullet : gs.playerBullets) {
            ...
            if(playerBulletBounds.findIntersection(bonusShipBounds).has_value()) {
                ...
                Explosion exp(0.2, gs.assets.explosion_texture, gs.bonusship.sprite.getPosition());
                if(gs.bonusship.lifes == 0) exp.sprite.setScale(sf::Vector2f(0.7, 0.7));
        ...
        if(gs.existsNuke && nukeBounds.findIntersection(bonusShipBounds).has_value()) {
            ...
            Explosion exp(0.7, gs.assets.explosion_texture, gs.nuke.sprite.getPosition()); 
            gs.nuke.sprite.setPosition(sf::Vector2f(0, -500));
            gs.explosions.push_back(exp);
        }
...
void moveEnemies(State& gs) {           
    ...
    float secondsToElapse = std::clamp(gs.enemiesQuantity/60.0, 0.09, 1.0); //con clamp definisco lim min e max di tempo da contare, divido per 60 come il num iniziale di nemici
    if(gs.enemiesQuantity == 1)secondsToElapse = 0.04; 

    if(gs.move_clock.getElapsedTime().asSeconds() >= secondsToElapse) {
        if(!gs.enemies.empty()) {
            float dist = std::clamp(gs.windowWidth * 0.78 / gs.enemiesQuantity,  gs.windowWidth * 0.018, gs.windowWidth * 0.021); //con clamp definisco lim min e max di tempo da contare
            bool edge = false;
